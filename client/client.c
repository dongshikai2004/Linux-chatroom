...
int main(){
    ...
    while(fgets(buf, MAXLINE, stdin) != NULL){
        ...
        if(buf[0] == ':'){                            //输入为命令语句
            ...
            else if(buf[1] == 'p'){ // 私聊功能
                char *username = strtok(buf + 2, " ");
                if(username == NULL || strtok(NULL, " ") == NULL) {
                    printf("私聊命令格式错误。\n");
                    continue;
                }
            }
            else if(buf[1] == 'h'){ // 帮助功能
                printf("可用命令:\n");
                printf("':q' - 退出聊天室\n");
                printf("':r' - 改名\n");
                printf("':s' - 显示所有在线用户\n");
...
void upload_file(){
    ...
    startlistening();      //发送结束，重新开启接收消息线程
