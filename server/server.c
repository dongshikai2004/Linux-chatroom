...
inline int Process(int ID){
	...
	//分离命令和参数，命令保存在op，参数保存在buf[ID]
	   ...
	   //服务器发送文件
        ...
        if(op[1] == 'p'){
	   //选择私聊某位用户
            char *targetName = strtok(buf[ID], " ");
            char *message = strtok(NULL, " ");
            int targetID = -1;
            for(int i = 0; i < MAXCON-1; i++) {
                if(used[i] && strcmp(names[i], targetName) == 0) {
                    targetID = i;
                    break;
                }
            }
            if(targetID == -1) {
                memset(spemsg[ID], 0, sizeof(spemsg[ID]));
                strcpy(spemsg[ID], "Error(7): 用户不存在，请重试。");
                sendonemsg(connfd[ID], spemsg[ID]);
                return 0;
            }
            sprintf(buf[ID], "(private) %s: %s", names[ID], message);
            sendonemsg(connfd[targetID], buf[ID]);
            memset(spemsg[ID], 0, sizeof(spemsg[ID]));
